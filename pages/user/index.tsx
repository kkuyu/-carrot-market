...
const UserIndexPage: NextPage = () => {
  ...
  const userButton = (options: { icon: React.ReactNode; title: string; pathname?: string | null; onClick?: () => void | null }) => {
    const buttonContent = () => {
      return (
        <div className="relative block py-1 pl-12 pr-5 [&>svg]:absolute [&>svg]:top-1.5 [&>svg]:left-5 [&>svg]:w-5 [&>svg]:h-5">
          {options.icon}
          <span>{options.title}</span>
        </div>
      );
    };
    if (!options?.pathname || options?.onClick) return null;
    if (options?.pathname) {
      return (
        <Link href={options.pathname}>
          <a className="block">{buttonContent()}</a>
        </Link>
      );
    }
    if (options?.onClick) {
      return (
        <button type="button" onClick={options.onClick} className="block w-full text-left">
          {buttonContent()}
        </button>
      );
    }
      ...
      links: [
        {
          ...
          content: userButton({
            title: "판매내역",
            pathname: `/profiles/${user?.id}/products`,
            icon: (
      ...
      links: [
        {
          ...
          content: userButton({
            title: "동네생활 글/댓글",
            pathname: `/profiles/${user?.id}/stories`,
            icon: (
              <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path
                  strokeLinecap="round"
