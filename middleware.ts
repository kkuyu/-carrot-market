  ...
  if (!isLogin) {
    switch (url.pathname) {
      ...
      default:
        ...
        if (/^\/stories\/[0-9]*$/.test(url.pathname)) return NextResponse.next();
        if (/^\/users\/profiles\/[0-9]*$/.test(url.pathname)) return NextResponse.next();
        if (/^\/users\/profiles\/[0-9]*\/products$/.test(url.pathname)) return NextResponse.next();
