...
const Thumbnails = ({ list, modal }: ThumbnailsProps) => {
  ...
  const makeImgTag = (item: ThumbnailItem, size: string, extraClass?: string) => {
    if (!modal) {
      return (
        <span className={`relative block w-full bg-slate-300 ${extraClass}`}>
          <span className={`block pb-[${size}]`}></span>
    ...
    return (
      <button type="button" className={`relative block w-full bg-slate-300 ${extraClass}`} onClick={() => openThumbnailModal(list, 0)}>
        <span className={`block pb-[${size}]`}></span>
        <Image src={`https://imagedelivery.net/QG2MZZsP6KQnt-Ryd54wog/${item.src}/public`} alt={item.name} layout="fill" objectFit="cover" />
      </button>
    );
  ...
  if (list.length === 1) {
    return <div className="relative overflow-hidden rounded-md">{makeImgTag(list[0], "40%")}</div>;
  }

  return (
    <div className="relative overflow-hidden rounded-md">
      <div className="grid grid-cols-2 gap-1">
        {list.slice(0, 3).map((item, index, array) => {
          return makeImgTag(item, array.length === 3 && index !== 0 ? "40%" : "80%", array.length === 3 && index === 0 ? "row-span-2" : "");
        })}
