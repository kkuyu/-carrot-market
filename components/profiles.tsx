...
const Profiles = ({ user, emdPosNm, diffTime, size = "base" }: ProfilesProps) => {
  const classNames = {
    sm: {
      wrapper: "",
      avatar: "w-11 h-11",
  ...
  return (
    <Link href={`/users/profiles/${user.id}`}>
      <a className={`flex items-center w-full ${classNames[size].wrapper}`}>
        ...
        <div className="grow pl-3">
          ...
          <span className={`block text-gray-500 ${classNames[size].extra}`}>{[emdPosNm, diffTime].filter((v) => !!v).join(" · ")}</span>
        </div>
        {/* <div>todo: 매너온도</div> */}
      </a>
    </Link>
